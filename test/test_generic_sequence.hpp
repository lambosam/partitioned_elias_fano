...
std::vector<uint64_t> random_sequence(size_t universe, size_t n, int seed,
                                      bool strict = true)
{
    srand(seed);
    std::vector<uint64_t> seq;
                          ...
                          std::vector<uint64_t> const& seq)
{
    ...
    // std::cout << "Builder size: " << bvb.size() << " BPK: " << (1.0 * bvb.size() / seq.size()) << std::endl;
    // std::cout << "Index size: " << bv.size() << " BPK: " << (1.0 * bv.size() / seq.size()) << std::endl;
    std::cout << "Uniform EF Index size (BPK): " << (1.0 * bv.size() / seq.size()) << std::endl;
}

template <typename ParamsType, typename SequenceType>
inline double cal_test_sequence(SequenceType,
                          ParamsType const& params,
                          uint64_t universe,
                          std::vector<uint64_t> const& seq)
{
